...
def render_upload_actions(ds: Dict[str, Any], connection: Dict[str, str]) -> None:
    ...
    if upload:
        ...
        if not connection.get("project_id"):
            st.error("Project ID is required. Set it in the sidebar before uploading.")
            return
        try:
            dataset_id, inserted, raw_body, create_attempts, item_attempts = upload_dataset_to_langfuse(ds, connection)
            ds["synced"] = True
            ...
            with st.expander("Upload debug"):
                st.write("Create attempts", create_attempts)
                st.write("Item attempts", item_attempts)
            ...
            st.caption(f"Tried item URLs under project: {connection.get('project_id')}")
...
def _post_with_fallback(
    ...
    """Try multiple URLs; return response plus debug info for each attempt."""
    ...
    for url in urls:
        ...
        try:
            ...
            attempts.append({"url": url, "status": resp.status_code, "text": resp.text})
            resp.raise_for_status()
            return resp, attempts
        except Exception as exc:  # pylint: disable=broad-except
            attempts.append({"url": url, "status": getattr(exc, 'response', None).status_code if hasattr(exc, 'response') and exc.response is not None else 'error', "text": str(exc)})
            last_err = exc
            continue
    if last_err:
        raise RuntimeError(f"All POST attempts failed; attempts={attempts}") from last_err
    raise RuntimeError("No valid URL provided for Langfuse request.")


def upload_dataset_to_langfuse(ds: Dict[str, Any], connection: Dict[str, str]) -> tuple[str, int, Any, List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    resp, create_attempts = _post_with_fallback(create_urls, headers, payload, timeout=20)
    created = resp.json()
    dataset_id = created.get("id") or created.get("datasetId") or created.get("name")
    dataset_name = created.get("name") or ds["name"]
    if not dataset_id and not dataset_name:
        raise RuntimeError("Langfuse response missing dataset id/name.")
    # Upload items (try project-scoped dataset path, then global, then dataset-items endpoints)
    items = []
    for item in ds["items"]:
        items.append(
            {
    ...
    # item endpoints sometimes expect dataset id, sometimes dataset name; try both.
    item_targets = [dataset_id, dataset_name]
    item_urls = [
        f"{base_url}/api/public/projects/{project_id}/datasets/{target}/items"
        for target in item_targets
    ] + [
        f"{base_url}/api/public/datasets/{target}/items"
        for target in item_targets
    ] + [
        f"{base_url}/api/public/projects/{project_id}/dataset-items" if project_id else None,
        f"{base_url}/api/public/dataset-items",
    ]
    # For dataset-items endpoint, payload is expected to be per-item not batched; adapt when hitting those URLs.
    item_attempts: List[Dict[str, Any]] = []
    resp_items: Optional[requests.Response] = None
    for url in item_urls:
        if not url:
            continue
        try:
            if url.endswith("/dataset-items"):
                # send items one by one with dataset name/id to maximize compatibility
                per_item_attempts = []
                for itm in items:
                    itm_payload = {
                        ...
                        "input": itm["input"],
                        "expectedOutput": itm["expectedOutput"],
                        "metadata": itm.get("metadata"),
                        "externalId": itm.get("externalId"),
                    }
                    r = requests.post(url, headers=headers, json=itm_payload, timeout=30)
                    per_item_attempts.append({"url": url, "status": r.status_code, "text": r.text})
                    r.raise_for_status()
                item_attempts.extend(per_item_attempts)
                resp_items = r  # last response
                break
            else:
                r = requests.post(url, headers=headers, json=items_payload, timeout=30)
                item_attempts.append({"url": url, "status": r.status_code, "text": r.text})
                r.raise_for_status()
                resp_items = r
                break
        except Exception as exc:  # pylint: disable=broad-except
            item_attempts.append(
                {
                    "url": url,
                    "status": getattr(exc, "response", None).status_code if hasattr(exc, "response") and exc.response is not None else "error",
                    "text": str(exc),
                }
            )
            continue
    if resp_items is None:
        raise RuntimeError(f"All item upload attempts failed; attempts={item_attempts}")
    # If still 4xx/5xx, surface content for debugging.
    ...
    return dataset_id, inserted, body, create_attempts, item_attempts
